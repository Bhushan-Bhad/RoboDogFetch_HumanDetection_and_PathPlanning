...
def human_detection():
    ...
    def process_frame(ros_image):
        ...
        frame = cv2.resize(frame, (640, 360))  # Adjust resolution to reduce processing


        # YOLOv8 to detect objects
        results = model(frame, classes=[0])
        if not results or not results[0].boxes:
           return  # No detections, skip processing
	    ...
	    #f results and results[0].boxes:	    
	    	#humans = [obj for obj in results[0].boxes if obj.cls[0].item() == 0]  # Only process class 0 (human)
	   # else:
	    #	humans = []
            ## Webcam specifications
            image_width = 640  # FHD resolution
                ...
                if hand_results.multi_hand_landmarks:
                    for hand_landmarks, hand_handedness in zip(hand_results.multi_hand_landmarks, hand_results.multi_handedness):
                        ...
                        if vertical and palm_open and fingers_open:
                            cv2.putText(frame, f"Hi Gesture Detected! )", (x1, y1 - 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    rospy.Subscriber("/private_camera/image_raw", Image, process_frame)
